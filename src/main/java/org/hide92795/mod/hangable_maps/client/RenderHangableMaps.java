...
import org.hide92795.mod.hangable_maps.HangableMaps.HangableMapData;
...
public class RenderHangableMaps extends Render {
	...
	public void renderHangableMap(EntityHangableMap entity, double var2, double var4, double var6, float var8,
		...
		if (trace) {
			...
			ClientEventHandler.maps.add(entity.getMapIdFromDataWatcher());
		}
		if (mapdata != null) {
	...
	public void renderMap(EntityHangableMap entity, EntityPlayer player, MapData mapdata) {
		...
		// render player icon
		HangableMapData mapinfo = HangableMaps.instance.getMapInformation((short) entity.getMapIdFromDataWatcher());
		textureManager.bindTexture(texture_map_icon);
		if (mapinfo.received) {
	...
	private void drawPlayer(EntityPlayer player, MapData mapdata, HangableMapData mapinfo) {
		if (player.dimension == mapinfo.dimension) {
			...
			if (HangableMaps.drawIconName) {
				...
				int width = fontRenderer.getStringWidth(player.getCommandSenderName());
				this.fontRenderer.drawString(player.getCommandSenderName(), -width / 2, 3, 0x000000);
				GL11.glPopMatrix();
			}
